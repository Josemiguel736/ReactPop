...
describe('NewAdvertPage', () => {
	...
	const renderComponent = (error?: Error | null, tagError?: ApiClientError) => {
		...
		return render(
			<Provider store={mockStore as Store<State>}>
				<NewAdvertPage />
			</Provider>,
		);
	};
	...
	test('Test to tags render error', async () => {
		const { container } = renderComponent(
			null,
			new ApiClientError('Not_found', 'NOT_FOUND'),
		);
	...
	test('Test to render error', async () => {
		...
		expect(alert).toHaveTextContent(
			'Ha ocurrido un problema al crear el producto porfavor intentalo más tarde',
		);
		expect(container).toMatchSnapshot();
	});
	...
	test('Test to minText error', async () => {
		...
		const alert = screen.getByRole('alert');
		expect(alert).toHaveTextContent(
			'El producto debe de tener al menos 3 letras',
		);
		const submitButton = screen.getByRole('button');
		await userEvent.click(submitButton);
